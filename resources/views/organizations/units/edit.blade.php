<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-primary leading-tight">
            ✏️ {{ __('Edit Department') }}: {{ $department->name }}
        </h2>
    </x-slot>
    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="surface shadow-md rounded-lg">
                <form method="POST" action="{{ route('organization.units.update', $department) }}">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            <div>
                                ...
                                <select id="type" name="type" required
                                    class="mt-1 block w-full border-secondary rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                                    <option value="department" {{ old('type', $department->type) == 'department' ? 'selected' : '' }}>Department</option>
                                    <option value="branch" {{ old('type', $department->type) == 'branch' ? 'selected' : '' }}>Branch</option>
                                    <option value="division" {{ old('type', $department->type) == 'division' ? 'selected' : '' }}>Division</option>
                                    <option value="head_office" {{ old('type', $department->type) == 'head_office' ? 'selected' : '' }}>Head Office</option>
                                </select>
                                @error('type')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <select id="parent_id" name="parent_id"
                                    ...
                                    @foreach($parentUnits as $unitOption)
                                        <option value="{{ $unitOption->id }}" {{ old('parent_id', $department->parent_id) == $unitOption->id ? 'selected' : '' }}>
                                            {{ $unitOption->name }}
                                        </option>
                                    @endforeach
                                </select>
