        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="mb-6 bg-red-50 border border-red-200 rounded-lg p-6">
                        ...
                        @foreach($outOfStockItems->take(6) as $item)
                            <div class="bg-white rounded-lg p-4 border border-red-300">
                                ...
                                <div class="mt-3 flex space-x-2">
                                    <x-button.link href="{{ route('inventory.transactions.create') }}?type=receipt&item_id={{ $item->id }}" size="sm">
                                        Receive Stock
                                    </x-button.link>
                    ...
                    @if($outOfStockItems->count() > 6)
                        <div class="mt-4 text-center">
                            <span class="text-red-700 font-medium">+{{ $outOfStockItems->count() - 6 }} more out of stock items</span>
                        </div>
                    @endif
                </div>
            @endif
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                            ...
                            <div>
                                ...
                                <x-form.select id="severity" name="severity" class="mt-1 block w-full">
                                    ...
                                    <option value="warning" {{ request('severity') == 'warning' ? 'selected' : '' }}>Warning (Low Stock)</option>
                                    <option value="info" {{ request('severity') == 'info' ? 'selected' : '' }}>Info (Near Reorder)</option>
                                </x-form.select>
                            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                ...
                <div class="bg-orange-50 p-4 rounded-lg border border-orange-200 text-center">
                    <div class="text-2xl font-bold text-orange-600">{{ $outOfStockItems->count() + $lowStockItems->count() }}</div>
                                    ...
                                    @foreach($lowStockItems as $item)
                                        <tr class="hover:bg-gray-50">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-red-600">
                                                -{{ max(0, $item->reorder_level - $item->total_quantity) }} {{ $item->unit }}
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    ...
                    @foreach($reorderSuggestions as $suggestion)
                        <div class="bg-white rounded-lg p-4 border border-blue-200">
                            ...
                            <div class="space-y-2 text-sm">
                                <div class="flex justify-between">
                                    <span>Current Stock:</span>
                                    <span class="font-medium">{{ $suggestion['current'] }} {{ $suggestion['item']->unit }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span>Reorder Level:</span>
                                    <span class="font-medium">{{ $suggestion['reorder_level'] }} {{ $suggestion['item']->unit }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span>Suggested Order:</span>
                                    <span class="font-bold text-blue-600">{{ $suggestion['suggested_order'] }} {{ $suggestion['item']->unit }}</span>
                                </div>
                            </div>
                            <div class="mt-3">
                                <x-button.primary href="{{ route('inventory.transactions.create') }}?type=receipt&item_id={{ $suggestion['item']->id }}" size="sm" class="w-full">
