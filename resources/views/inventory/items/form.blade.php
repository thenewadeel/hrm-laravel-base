        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form
                        action="{{ isset($item) ? route('inventory.items.update', $item) : route('inventory.items.store') }}"
                        method="POST">
                        @csrf
                        @if (isset($item))
                            @method('PUT')
                        ...
                        <div class="space-y-8">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div class="md:col-span-2">
                                        ...
                                        <x-form.input id="name" name="name" type="text"
                                            class="mt-1 block w-full" :value="old('name', $item->name ?? '')" required autofocus />
                                        <x-form.input-error for="name" class="mt-2" />
                                    </div>

                                    <div>
                                        <x-form.label for="sku" value="SKU *" />
                                        <x-form.input id="sku" name="sku" type="text"
                                            class="mt-1 block w-full" :value="old('sku', $item->sku ?? '')" required />
                                        <x-form.input-error for="sku" class="mt-2" />
                                    </div>

                                    <div>
                                        <x-form.label for="unit" value="Unit of Measure *" />
                                        <x-form.input id="unit" name="unit" type="text"
                                            class="mt-1 block w-full" :value="old('unit', $item->unit ?? 'pcs')" required />
                                    ...
                                    <div class="md:col-span-2">
                                        ...
                                        <x-form.textarea id="description" name="description" class="mt-1 block w-full"
                                            rows="3"
                                            placeholder="Describe this item...">{{ old('description', $item->description ?? '') }}</x-form.textarea>
                                    ...
                                    <div>
                                        ...
                                        <x-form.select id="category" name="category" class="mt-1 block w-full">
                                            ...
                                            <option value="Hardware"
                                                {{ old('category', $item->category ?? '') == 'Hardware' ? 'selected' : '' }}>
                                                Hardware</option>
                                            <option value="Electrical"
                                                {{ old('category', $item->category ?? '') == 'Electrical' ? 'selected' : '' }}>
                                                Electrical</option>
                                            <option value="Tools"
                                                {{ old('category', $item->category ?? '') == 'Tools' ? 'selected' : '' }}>
                                                Tools</option>
                                            <option value="Automotive"
                                                {{ old('category', $item->category ?? '') == 'Automotive' ? 'selected' : '' }}>
                                                Automotive</option>
                                            <option value="Office Supplies"
                                                {{ old('category', $item->category ?? '') == 'Office Supplies' ? 'selected' : '' }}>
                                                Office Supplies</option>
                                            <option value="Safety Equipment"
                                                {{ old('category', $item->category ?? '') == 'Safety Equipment' ? 'selected' : '' }}>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <x-form.input id="cost_price" name="cost_price" type="number" step="1"
                                            min="0" class="mt-1 block w-full" placeholder="210 for $2.10"
                                    ...
                                    <div>
                                        ...
                                        <x-form.input id="selling_price" name="selling_price" type="number"
                                            step="1" min="0" class="mt-1 block w-full"
                                            placeholder="250 for $2.50" :value="old('selling_price', $item->selling_price ?? '')" />
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <x-form.input id="reorder_level" name="reorder_level" type="number"
                                            min="0" class="mt-1 block w-full" :value="old('reorder_level', $item->reorder_level ?? 0)" />
                                        <x-form.input-error for="reorder_level" class="mt-2" />
                                    </div>

                                    <div class="flex items-center md:items-end">
                                        <x-form.checkbox id="is_active" name="is_active" :checked="old('is_active', $item->is_active ?? true)" />
                            ...
                            <div>
                                <x-form.input id="organization_id" name="organization_id" class="hidden"
                                    :value="old('organization_id', auth()->user()->operatingOrganizationId)" />
                            </div>
                        </div>
