    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-primary leading-tight">
                🏪 {{ __('Stores') }} <span class="text-muted text-lg">({{ $stores->total() }} stores)</span>
            </h2>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                            ...
                            <div class="flex-1">
                                ...
                                <div class="mt-1 relative rounded-md shadow-sm">
                                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                        <x-heroicon-s-magnifying-glass class="h-4 w-4 text-muted" />
                                    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @forelse($stores as $store)
                    <div class="surface overflow-hidden shadow-sm sm:rounded-lg border border-secondary hover:shadow-md transition-shadow duration-200"
                        title={{ $store }}>
                        <div class="p-6">
                            <!-- Store Header -->
                            <div class="flex items-center justify-between mb-4">
                                <div class="flex items-center">
                                    ...
                                    <div class="ml-4">
                                        <h3 class="text-lg font-semibold text-primary">{{ $store->name }}</h3>
                            ...
                            <div class="space-y-3 text-sm text-secondary mb-4">
                                @if ($store->location)
                                    <div class="flex items-center">
                                        <x-heroicon-s-map-pin class="h-4 w-4 mr-2 text-muted" />
                                ...
                                @if ($store->organizationUnit)
                                    <div class="flex items-center">
                                        <x-heroicon-s-building-office class="h-4 w-4 mr-2 text-muted" />
                                        <span>{{ $store->organizationUnit->name }}</span>
                                    </div>
                                @endif

                                <div class="flex items-center">
                                    <x-heroicon-s-cube class="h-4 w-4 mr-2 text-muted" />
                                    <span>{{ $store->items_count ?? 0 }} items</span>
                                </div>

                                @if ($store->description)
                                    <div class="pt-2 border-t border-secondary">
                                        <p class="text-muted text-xs">{{ Str::limit($store->description, 100) }}</p>
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="bg-tertiary rounded-lg p-3 mb-4">
                                <div class="grid grid-cols-2 gap-4 text-xs">
                                    <div class="text-center">
                                        <div class="font-semibold text-primary">Total Value</div>
                                        <div class="text-green-600 font-medium">
                                            ${{ number_format(($store->total_value ?? 0) / 100, 2) }}</div>
                                    </div>
                                    <div class="text-center">
                                        <div class="font-semibold text-primary">Low Stock</div>
                ...
                @empty
                    <div class="col-span-3">
                        <div class="text-center py-12">
                            <x-heroicon-s-building-storefront class="mx-auto h-16 w-16 text-muted" />
                            <h3 class="mt-4 text-lg font-medium text-primary">No stores found</h3>
                            <p class="mt-2 text-sm text-muted">Get started by creating your first store location.</p>
                            <div class="mt-6">
                                <x-button.primary href="{{ route('inventory.stores.create') }}">
                                    <x-heroicon-s-plus class="w-4 h-4 mr-2" />
                                    Add Store
