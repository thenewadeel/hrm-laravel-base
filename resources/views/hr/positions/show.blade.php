        ...
        <div class="flex justify-between items-center">
            <div>
                <h2 class="font-semibold text-xl text-primary leading-tight">
                    🏢 {{ __('Position Management > Position Details') }}
                </h2>
                <p class="text-sm text-secondary mt-1">
                    View job position details and assigned employees
                </p>
            </div>
        </div>
    </x-slot>
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="mt-1 flex flex-col sm:flex-row sm:flex-wrap sm:mt-0 sm:space-x-6">
                        ...
                        @if ($position->created_at)
                            <div class="mt-2 flex items-center text-sm text-secondary">
                                Created: {{ $position->created_at->format('F j, Y') }}
                            </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="surface shadow overflow-hidden sm:rounded-lg">
                        ...
                        <div class="px-4 py-5 sm:p-6">
                            <dl class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                                <div>
                                    <dt class="text-sm font-medium text-secondary">Title</dt>
                                    <dd class="mt-1 text-sm text-primary">{{ $position->title }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-secondary">Code</dt>
                                    <dd class="mt-1 text-sm text-primary font-mono">{{ $position->code }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-secondary">Department</dt>
                                    <dd class="mt-1 text-sm text-primary">{{ $position->organizationUnit?->name ?? 'No Department' }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-secondary">Salary Range</dt>
                                ...
                                @if ($position->description)
                                    <div class="sm:col-span-2">
                                        ...
                                        <dd class="mt-1 text-sm text-primary">{{ $position->description }}</dd>
                                    </div>
                                @endif
                                @if (! empty($position->requirements))
                                    <div class="sm:col-span-2">
                                        <dt class="text-sm font-medium text-secondary">Requirements</dt>
                                        <dd class="mt-1 text-sm text-primary">
                                            {{ is_array($position->requirements) ? implode(', ', $position->requirements) : $position->requirements }}
                                        </dd>
                                    </div>
                                @endif
                            </dl>
                    ...
                    <div class="surface shadow overflow-hidden sm:rounded-lg">
                        ...
                        <div class="px-4 py-5 sm:p-6">
                            @if (! empty($position->default_roles))
                                <div class="flex flex-wrap gap-2">
                                    @foreach($position->default_roles as $role)
                                ...
                                <p class="mt-3 text-sm text-secondary">
                                    These roles are automatically granted to employees assigned this position.
                                </p>
                            @else
                                <p class="text-sm text-secondary">No default roles configured.</p>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="surface shadow overflow-hidden sm:rounded-lg">
                        <div class="px-4 py-5 sm:px-6 border-b border-secondary">
                            <h3 class="text-lg leading-6 font-medium text-primary">
                                Assigned Employees ({{ $position->employees->count() }})
                            </h3>
                        </div>
                        <div class="px-4 py-5 sm:p-6">
                            @if($position->employees->count() > 0)
                                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                                    @foreach($position->employees as $employee)
                                        <a href="{{ route('hr.employees.show', $employee) }}" class="border border-secondary rounded-lg p-4 hover:bg-tertiary block">
                            ...
                            @else
                                <div class="text-center py-8">
                                    ...
                                    <p class="mt-1 text-sm text-secondary">No employees are currently assigned to this position.</p>
                                </div>
                            @endif
                        </div>
                    </div>
                ...
                <div class="space-y-6">
                        ...
                        <div class="px-4 py-5 sm:p-6">
                            <dl class="space-y-4">
                                ...
                                <div class="flex justify-between">
                                    ...
                                    <dd class="text-sm text-primary">{{ $position->employees->count() }}</dd>
                                </div>
                                <div class="flex justify-between">
                                    <dt class="text-sm font-medium text-secondary">Default Roles</dt>
                                    <dd class="text-sm text-primary">{{ count($position->default_roles ?? []) }}</dd>
                                </div>
                            </dl>
                        </div>
                    </div>
                    ...
                    <div class="surface shadow overflow-hidden sm:rounded-lg">
                        ...
                        <div class="px-4 py-5 sm:p-6 space-y-3">
                            ...
                            <form action="{{ route('hr.positions.destroy', $position) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this position?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
