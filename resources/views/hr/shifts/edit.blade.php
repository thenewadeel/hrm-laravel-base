        ...
        <div class="flex justify-between items-center">
            <div>
                ...
                <p class="text-sm text-secondary mt-1">
                    Update shift schedule and timings
                </p>
            </div>
        </div>
    </x-slot>
                    ...
                    <div class="p-6">
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="code" id="code" value="{{ old('code', $shift->code) }}"
                                       class="mt-1 block w-full border-secondary rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                                       required>
                                @error('code')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="time" name="start_time" id="start_time" value="{{ old('start_time', optional($shift->start_time)->format('H:i')) }}"
                                           class="mt-1 block w-full border-secondary rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                                           required>
                                    @error('start_time')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                ...
                                <div>
                                    ...
                                    <input type="time" name="end_time" id="end_time" value="{{ old('end_time', optional($shift->end_time)->format('H:i')) }}"
                                           class="mt-1 block w-full border-secondary rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                                           required>
                                    @error('end_time')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            ...
                            <!-- Working Hours -->
                            <div>
                                <label for="working_hours" class="block text-sm font-medium text-primary">Working Hours</label>
                                <input type="number" name="working_hours" id="working_hours" value="{{ old('working_hours', $shift->working_hours) }}" min="1" max="24"
                                       class="mt-1 block w-full border-secondary rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                                       required>
                                @error('working_hours')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            ...
                            <!-- Days of Week -->
                            <div>
                                <label class="block text-sm font-medium text-primary mb-2">Days of Week</label>
                                <div class="grid grid-cols-2 gap-2">
                                    @foreach(['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'] as $day)
                                        <div class="flex items-center">
                                            <input type="checkbox" name="days_of_week[]" id="day_{{ $day }}" value="{{ $day }}"
                                                   {{ in_array($day, old('days_of_week', $shift->days_of_week ?? [])) ? 'checked' : '' }}
                                                   class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-secondary rounded">
                                            <label for="day_{{ $day }}" class="ml-2 block text-sm text-primary capitalize">
                                                {{ $day }}
                                            </label>
                                        </div>
                                    @endforeach
                            ...
                            <div>
                                ...
                                <textarea name="description" id="description" rows="4"
                                          class="mt-1 block w-full border-secondary rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">{{ old('description', $shift->description) }}</textarea>
                                @error('description')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <!-- Active Status -->
                            <div class="flex items-center">
                                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $shift->is_active) ? 'checked' : '' }}
                                       class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-secondary rounded">
