<div>
    <!-- Header -->
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Accounts Receivable Outstanding</h2>
        <p class="text-gray-600 dark:text-gray-400 mt-1">Customer outstanding balances with aging analysis</p>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <div>
                ...
                <select wire:model.live="customerId" class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse($statement['customer_statements'] as $customerStatement)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-900 dark:text-gray-300">
                                    @if($customerStatement['customer']['email'])
                                        <div>{{ $customerStatement['customer']['email'] }}</div>
                                    @endif
                                    @if($customerStatement['customer']['phone'])
                                        <div class="text-gray-500">{{ $customerStatement['customer']['phone'] }}</div>
                                    @endif
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium text-gray-900 dark:text-white">
                                {{ number_format($customerStatement['total_outstanding'], 2) }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm text-green-600">
                                {{ number_format($customerStatement['aging']['current'], 2) }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm text-yellow-600">
                                {{ number_format($customerStatement['aging']['30_days'], 2) }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm text-orange-600">
                                {{ number_format($customerStatement['aging']['60_days'], 2) }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm text-red-600">
                                {{ number_format($customerStatement['aging']['90_days'], 2) }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-left text-sm font-medium">
                                <button wire:click="showCustomerDetails({{ $customerStatement }})" 
                ...
                <div class="mt-3">
                    ...
                    <div class="bg-gray-50 dark:bg-gray-900 rounded-lg p-4 mb-4">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <div class="font-medium">{{ $selectedCustomer['customer']['email'] ?? 'N/A' }}</div>
                            </div>
                            <div>
                                <div class="text-sm text-gray-600 dark:text-gray-400">Phone</div>
                                <div class="font-medium">{{ $selectedCustomer['customer']['phone'] ?? 'N/A' }}</div>
                            </div>
                            <div class="col-span-2">
                                <div class="text-sm text-gray-600 dark:text-gray-400">Address</div>
                                <div class="font-medium">{{ $selectedCustomer['customer']['address'] ?? 'N/A' }}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-4">
                        <div class="text-center">
                            ...
                            <div class="text-lg font-bold">{{ number_format($selectedCustomer['total_outstanding'], 2) }}</div>
                        </div>
                        <div class="text-center">
                            <div class="text-sm text-gray-600 dark:text-gray-400">Current</div>
                            <div class="text-lg font-bold text-green-600">{{ number_format($selectedCustomer['aging']['current'], 2) }}</div>
                        </div>
                        <div class="text-center">
                            <div class="text-sm text-gray-600 dark:text-gray-400">30 Days</div>
                            <div class="text-lg font-bold text-yellow-600">{{ number_format($selectedCustomer['aging']['30_days'], 2) }}</div>
                        </div>
                        <div class="text-center">
                            <div class="text-sm text-gray-600 dark:text-gray-400">60 Days</div>
                            <div class="text-lg font-bold text-orange-600">{{ number_format($selectedCustomer['aging']['60_days'], 2) }}</div>
                        </div>
                        <div class="text-center">
                            <div class="text-sm text-gray-600 dark:text-gray-400">90+ Days</div>
                            <div class="text-lg font-bold text-red-600">{{ number_format($selectedCustomer['aging']['90_days'], 2) }}</div>
                        </div>
                    </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                @foreach($selectedCustomer['entries'] as $entry)
                                    <tr>
