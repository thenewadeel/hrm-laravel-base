<div class="p-6">
    ...
    <script>
        function toggleAllSubscriptions() {
            ...
            @this.set('selectedSubscriptions', Array.from(checkboxes).map(cb => cb.value));
            @this.set('showBulkActions', selectAll.checked);
        }
    </script>
    @endscript
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 mb-8">
        <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Revenue Trend (Last 6 Months)</h3>
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div>
                <div class="space-y-2">
                    @foreach($revenueData as $data)
                        <div class="flex justify-between items-center">
                            <span class="text-sm text-gray-600 dark:text-gray-400">{{ $data['month'] }}</span>
                            <span class="text-sm font-medium text-gray-900 dark:text-white">${{ number_format($data['revenue'], 2) }}</span>
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            <div>
                <h4 class="text-sm font-medium text-gray-900 dark:text-white mb-3">Plan Distribution</h4>
                <div class="space-y-2">
                    @foreach($planDistribution as $planName => $count)
                        <div class="flex justify-between items-center">
                            <span class="text-sm text-gray-600 dark:text-gray-400">{{ $planName }}</span>
                            <span class="text-sm font-medium text-gray-900 dark:text-white">{{ $count }} members</span>
                        </div>
                    @endforeach
                </div>
            </div>
    ...
    <!-- Country Club Subscription Plans -->
    <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 mb-8">
        <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Country Club Membership Plans</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($subscriptionPlans as $plan)
                <div class="border border-gray-200 dark:border-gray-600 rounded-lg p-4 hover:shadow-lg transition-shadow">
                    ...
                    <div class="mb-4">
                        <div class="text-2xl font-bold text-indigo-600 dark:text-indigo-400">
                            ...
                            <span class="text-sm font-normal text-gray-500 dark:text-gray-400">/{{ $plan->billing_frequency }}</span>
                        </div>
                        @if($plan->family_members_included > 1)
                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                Includes {{ $plan->family_members_included }} family members
                            </p>
                        @endif
                    </div>

                    <div class="mb-4">
                        <h5 class="text-sm font-medium text-gray-900 dark:text-white mb-2">Benefits:</h5>
                    ...
                    <div class="flex justify-between items-center pt-3 border-t border-gray-200 dark:border-gray-600">
                        <span class="text-xs text-gray-500 dark:text-gray-400">
                            {{ $plan->billing_frequency === 'yearly' ? 'Annual' : 'Monthly' }} billing
                        </span>
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center space-x-4">
                <span class="text-sm text-blue-800 dark:text-blue-200">
                    {{ count($selectedSubscriptions) }} subscription(s) selected
                </span>
                @if(auth()->user()->can('membership.renew_subscriptions'))
                    <button wire:click="bulkRenew" class="px-3 py-1 bg-green-600 text-white text-sm rounded hover:bg-green-700">
                        Bulk Renew
                    </button>
                @endif
                @if(auth()->user()->can('membership.manage_subscriptions'))
                    <button wire:click="bulkSendReminders" class="px-3 py-1 bg-yellow-600 text-white text-sm rounded hover:bg-yellow-700">
                        Send Reminders
                    </button>
                @endif
            </div>
            <button wire:click="clearSelection" class="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300">
                Clear Selection
            </button>
        </div>
    </div>
        ...
        <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center space-x-2">
                    @if(auth()->user()->can('membership.export_data'))
                        <button wire:click="exportSubscriptions" class="px-3 py-1 bg-gray-600 text-white text-sm rounded hover:bg-gray-700">
                            Export CSV
                        </button>
                    @endif
                    <button wire:click="selectAllSubscriptions" class="px-3 py-1 bg-indigo-600 text-white text-sm rounded hover:bg-indigo-700">
                        Select All
                    </button>
                </div>
            </div>
                ...
                <thead class="bg-gray-50 dark:bg-gray-700">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                            <input type="checkbox" wire:model.live="selectAll" onchange="toggleAllSubscriptions()" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
                        </th>
                    ...
                    @forelse($subscriptions as $subscription)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <input 
                                    type="checkbox" 
                                    wire:click="toggleSubscriptionSelection({{ $subscription->id }})"
                                    {{ in_array($subscription->id, $selectedSubscriptions) ? 'checked' : '' }}
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                                <div>${{ number_format($subscription->total_amount, 2) }}</div>
    ...
    <!-- Payment Modal -->
    @if($showPaymentForm)
        <div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50" wire:click="hidePaymentForm">
            <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-lg bg-white dark:bg-gray-800" wire:click.stop>
                <div class="mt-3">
                    <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Process Payment</h3>
                    ...
                    <form wire:submit="processPayment">
                        <div class="mb-4">
                            ...
                            <input type="number" wire:model="payment_amount" step="0.01" min="0.01" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white">
                        ...
                        <div class="mb-4">
                            ...
                            <select wire:model="payment_method" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white">
                        ...
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Reference Number (Optional)</label>
                            <input type="text" wire:model="payment_reference" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white" placeholder="Transaction reference...">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea wire:model="payment_notes" rows="3" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white" placeholder="Additional payment notes..."></textarea>
                            @error('payment_notes')
                                <span class="text-red-600 dark:text-red-400 text-xs">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="flex justify-end space-x-3">
                            <button type="button" wire:click="hidePaymentForm" class="px-4 py-2 bg-gray-300 text-gray-700 rounded-lg hover:bg-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-500">
                                Cancel
                            </button>
